...
Batch process all supported image files (JPG, PNG, BMP) in `phone_image`
including nested subdirectories, and write outputs to `phone_border`:
...
This also creates phone-region slices in `phone_image_clip` by default (same as `-c`).
...
- Default (no mode flag): same as `-c`
...
Use either positional output dir or `--output-dir` (not both).
...
3. Check results in `phone_border` (mirrors input subdirectories):
