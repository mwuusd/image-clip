...
def run() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        default="phone_border",
        help="Directory to write border .txt files.",
    )
    parser.add_argument(
        "--image",
        default=None,
    ...
    parser.add_argument(
        "--clip-output-dir",
        default="phone_image_clip",
        help="Directory to write sliced phone-region images.",
    )
    parser.add_argument(
        "--slice-size",
        type=int,
    ...
    if args.input_dir_positional is not None and args.input_dir != "phone_image":
        print("Provide input directory via either positional path or --input-dir, not both.")
        return 1
    if args.output_dir_positional is not None and args.output_dir != "phone_border":
        print("Provide output directory via either positional path or --output-dir, not both.")
        return 1

    raw_input_dir = args.input_dir_positional or args.input_dir
    raw_output_dir = args.output_dir_positional or args.output_dir
    input_dir = Path(raw_input_dir).expanduser()
    output_dir = Path(raw_output_dir).expanduser()
    clip_output_dir = Path(args.clip_output_dir).expanduser()

    do_clip = args.clip or not args.border_only
